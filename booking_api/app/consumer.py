...
from database.database import consumer_db_session  # Now using the SQLModel session
    ...
    try:
        ...
        with consumer_db_session() as session:
            if event["event"] == "apartment_added":
                ...
                apartment = session.get(Apartment, event["data"]["apartment_id"])
                if apartment:
                    session.delete(apartment)
                    session.commit()
                    print(f"Apartment {event['data']['id']} removed from the database.", flush=True)
                else:
                    print(f"Apartment {event['data']['id']} not found.", flush=True)

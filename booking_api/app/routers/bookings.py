...
def publish_event(event_name, data):
    ...
    channel.basic_publish(
        exchange="booking_events", routing_key="", body=message
    )
    connection.close()
...
def add_booking(booking: BookingIn, session: SessionDep):
    ...
    check_availability(
        booking.apartment_id, booking.start_date, booking.end_date, session
    )
...
def change_booking(
    booking_id: str, booking: BookingUpdate, session: SessionDep
):
    ...
    check_availability(
        booking_db.apartment_id, start_date, end_date, session, booking_id
    )

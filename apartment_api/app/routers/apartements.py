...
def publish_event(event_name, data):
    ...
    channel.basic_publish(exchange='apartment_events', routing_key="", body=message)
    connection.close()

@router.post("/", status_code=201)
def add_apartment(apartment: ApartmentIn, session: SessionDep):
    """Add a new apartment"""
    apartments = session.exec(select(Apartment).where(Apartment.name == apartment.name)).all()
    
    if len(apartments):
        raise HTTPException(status_code=403, detail="Object already exists")
    
    new_apartment = Apartment.model_validate(apartment)  # Convert Pydantic model to SQLModel
